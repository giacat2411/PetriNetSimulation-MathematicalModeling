...
class Item1_bi(PetriNet):        
    ...
    def setup(self):
        ...
        if self.flag_auto == 1:
            ...
            if (not (self.input_free.get().isnumeric() and self.input_busy.get().isnumeric()
                ...
                tkinter.messagebox.showwarning('Lỗi nhập', 'Mỗi vị trí không chứa quá 1 token do yêu cầu đề')
                print("FAILED\n"  + "-"*30)
    ...
    def onClick(self, event):
        if (self.init_busy == "" and self.init_free == "" and self.init_docu == ""):
            tkinter.messagebox.showwarning('Lỗi', 'Vui lòng bấm SETUP để khởi tạo Marking ban đầu !!!')
        elif (self.flag_auto == 1):
            ...
            if tag == "start" and self.flag_fire_start != 1:
                if self.free != 0:
                    if self.busy > 0:
                        tkinter.messagebox.showwarning('Lỗi', 'Không thể tích hoạt vì sau đó token ở Busy = 2')
                    else:
                        print("FIRING START!!!")
                        self.fire_start()
                else:
                ...
                if self.docu != 0:
                    if self.free > 0:
                        tkinter.messagebox.showwarning('Lỗi', 'Không thể tích hoạt vì sau đó token ở Free = 2')
                    else:
                        print("FIRING END!!!")
                        self.fire_end()
                else:
                ...
                if self.busy != 0:
                    if self.docu > 0:
                        tkinter.messagebox.showwarning('Lỗi', 'Không thể tích hoạt vì sau đó token ở Docu = 2')
                    else:
                        print("FIRING CHANGE!!!")
                        self.fire_change()
                else:
